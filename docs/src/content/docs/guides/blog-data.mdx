...
The `starlightBlogs` map is keyed by blog [`prefix`](/configuration/#prefix) and contains all the blog data for each [blog instance](/guides/multiple-instances/).
Each value is a [`starlightBlog`](#starlightblog) object with the same properties as described above.

...
3. Starlight Blog uses Astro’s [content collections](https://docs.astro.build/en/guides/content-collections/), which are configured in the `src/content.config.ts` file.
   ...
   If you are already [extending Starlight’s schema](https://starlight.astro.build/reference/frontmatter/#extend) with additional fields, you can rely on [Zod's API](https://v3.zod.dev/?id=merge) to compose schemas together, e.g. `blogSchema(context).merge(mySchema)`.
